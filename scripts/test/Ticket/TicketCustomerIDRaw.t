...
for ( 1 .. 3 ) {
    my $RandomString = 'CustomerTestName + '.$HelperObject->GetRandomID();
    ...
    $Self->True(
        $ID,
        "Created company $RandomString-Name with id $ID",
    );
...
for my $CustomerID (@CustomerCompanyIDs) {
    ...
    my $Update = $CustomerCompanyObject->CustomerCompanyUpdate(
        ...
        CustomerCompanyCity    => 'Novi Sad',
        CustomerCompanyCountry => 'Serbia',
    ...
    @ReturnedTicketIDs = $TicketObject->TicketSearch(
        Result     => 'ARRAY',
        CustomerIDRaw => "$CustomerID  %  updated",
        UserID     => 1,
        OrderBy    => ['Up'],
        SortBy     => ['TicketNumber'],
    );
