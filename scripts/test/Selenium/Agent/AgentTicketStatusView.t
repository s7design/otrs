...
$Selenium->RunTest(
    sub {
        ...
        # create test tickets
        my @TicketIDs;
        for my $TicketCreate (qw (One Two Three) ) {
            my $TicketTitle = $TicketCreate . $Helper->GetRandomID();
            ...
            $Self->True(
                $TicketID,
                "TicketCreate() successful for test Ticket $TicketCreate ID $TicketID",
            );
        ...
        for my $Filter (qw(Open Closed)) {
            ...
            my $Element = $Selenium->find_element("//a[contains(\@href, \'Action=AgentTicketStatusView;SortBy=Age;OrderBy=Down;View=;Filter=$Filter\' )]");
            $Element->is_enabled();
            $Element->is_displayed();
            $Element->click();

            # check different views on screen
            for my $View (qw(Small Medium Preview)) {

                # click on filter controler
                $Selenium->find_element("//a[contains(\@href, \'Action=AgentTicketStatusView;Filter=$Filter;View=$View;\' )]")->click();
