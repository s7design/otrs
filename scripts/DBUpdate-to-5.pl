...
sub _MigrateConfigs {
    ...
    for my $CustomerUserGenericModule ( sort keys %{$Setting} ) {
        ...
        if ( $Module !~ m{Kernel::Output::HTML::Customer(\w+)} ) {
            next CUSTOMERUSERGENERICMODULE;
        }
        $Module =~ s{Kernel::Output::HTML::Customer(\w+)}{Kernel::Output::HTML::Customer::$1}xmsg;
    ...
    my $Success = $SysConfigObject->ConfigItemUpdate(
        ...
        Value => 'Kernel::Output::HTML::Customer::NewTicketQueueSelectionGeneric',
    );
